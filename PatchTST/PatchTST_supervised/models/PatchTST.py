...
from PatchTST.PatchTST_supervised.layers.PatchTST_backbone import PatchTST_backbone
from PatchTST.PatchTST_supervised.layers.PatchTST_layers import series_decomp
...
class Model(nn.Module):
    ...
    def forward(self, x):  # x: [Batch, Input length, Channel]
        if self.decomposition:
            ...
            x = self.model(x)
            x = x.permute(0, 2, 1)  # x: [Batch, pred_len, Channel]
        return x
